...
def _render_ruo_notice(pdf: LuminousPDF):
    ...
    pdf.multi_cell(180, 3.2,
        ...
        "ChEMBL/Open Targets drug data, Anthropic Claude AI interpretation, Gemini Veo protein "
        "animation, and Wiley Scholar Gateway full-text journal access to produce comprehensive "
        "protein structure analysis reports with per-residue confidence scoring."
    )
    pdf.ln(2)
...
def _render_query_box(pdf: LuminousPDF, query: ProteinQuery, report_id: str):
    ...
    fields = [
        ...
        ("Compute", "Tamarind Bio Cloud / Modal H100 GPU"),
    ]
...
def _render_partners_strip(pdf: LuminousPDF):
    ...
    partners = ["Tamarind Bio", "Anthropic Claude", "BioRender", "Gemini Veo", "Wiley Scholar", "Modal", "MolViewSpec", "BioMCP"]
    total_w = 180
    sep = " | "
...
def _render_methodology(pdf: LuminousPDF, query: ProteinQuery, prediction: PredictionResult):
    ...
    pdf.subsection_header("Protein Animation")
    pdf.body_text(
        "Cinematic protein animations are generated by Google Veo (Gemini API), using "
        "structure screenshots as input to produce short AI-generated videos of the "
        "predicted 3D structure."
    )

    pdf.subsection_header("Literature Access")
    pdf.body_text(
        "Full-text journal articles are accessed via the Wiley Scholar Gateway MCP "
        "connector, providing on-demand retrieval of primary research publications "
        "to support AI-generated interpretations."
    )
    ...
    sw = [
        ...
        "Anthropic Claude AI interpretation + MCP connectors",
        "Google Gemini Veo protein animation",
        "Wiley Scholar Gateway full-text journal access",
        "BioMCP variant and context retrieval (PubMed, ChEMBL, ClinVar, Open Targets)",
        "BioRender scientific illustration templates",
        "Modal serverless GPU compute (H100/A100)",
...
def _render_disclaimer(pdf: LuminousPDF, report_id: str):
    ...
    pdf.multi_cell(174, 3,
        ...
        "Drug information from ChEMBL and Open Targets. Protein animations generated by "
        "Google Gemini Veo. Journal articles accessed via Wiley Scholar Gateway. Database "
        "content may not reflect the most recent updates."
    )
    pdf.set_xy(18, y + 27)
    pdf.set_font(pdf._FONT, "", 6.5)
    pdf.cell(0, 3,
    ...
    pdf.set_font(pdf._FONT, "B", 7)
    pdf.cell(0, 3,
        "Powered by: Tamarind Bio | Anthropic Claude | BioRender | Gemini Veo | Wiley Scholar | Modal | MolViewSpec | BioMCP"
    )
